...
$(document).ready(function () {
  ...
  function displayBreed(image) {
    ...
    $('#breed-height').text(
      'Height: ' + image.breeds[0].height.metric + ' inches'
    );
    $('#breed-weight').text(
      'Weight: ' + image.breeds[0].weight.metric + ' lbs'
    );
      ...
      .then(function (data) {
        ...
        for (i = 0; i < dogArray.length; i++) {
          ...
          var card = $('<div>').addClass(
            'max-w-sm rounded bg-green-50 overflow-hidden shadow-lg'
          );
          var img = $('<img>')
            .attr('src', animal.primary_photo_cropped.small)
            .addClass('w-full')
          ...
          var span3 = $('<span>')
            .addClass(
              'inline-block bg-green-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2 breedID'
            )
            .text(animal.breeds.primary);
          var moreInfo = $("<a href ='" + animal.url + "'>")
            .attr('target', '_blank')
            .addClass('py-2')
            .css('color', 'blue')
            .text('More info...');
